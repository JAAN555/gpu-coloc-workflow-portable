...
def main():
    ap = argparse.ArgumentParser(
        description="Build chromosome-level grouped eQTL parquet files"
    )
    ...
    ap.add_argument(
        "--eqtl-items",
        required=True,
        help="Items formatted as EQTL_ID=ROOT|||EQTL_ID=ROOT"
    )
    ...
    chrom_target = clean_chromosome(args.chrom)
    items = [x for x in args.eqtl_items.split("|||") if x]
    ...
    idx = idx.sort_values(
        ["location_min", "location_max", "signal"]
