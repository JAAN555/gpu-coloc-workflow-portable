...
echo "comparison_id: Astle_2016_m1e6_vs_tx" > "Astle_2016_m1e6_vs_tx.log"
echo "gwas_name: Astle_2016_m1e6" >> "Astle_2016_m1e6_vs_tx.log"
echo "eqtl_name: tx" >> "Astle_2016_m1e6_vs_tx.log"
echo "dir1: /gpfs/helios/home/jaanotte/gpu-coloc-workflow-portable/nf_out_prepare_astle_parquets_modular_36_fillminus1e6/03_gwas_grouped_root/gwas_grouped" >> "Astle_2016_m1e6_vs_tx.log"
echo "dir2: /gpfs/helios/projects/shared_gwas/gpu-coloc_resources/eQTL_Catalogue/tx_parquets" >> "Astle_2016_m1e6_vs_tx.log"
echo "p12: 1e-6" >> "Astle_2016_m1e6_vs_tx.log"
echo "H4: 0.8" >> "Astle_2016_m1e6_vs_tx.log"
echo "task_host: $(hostname)" >> "Astle_2016_m1e6_vs_tx.log"
echo "task_start: $(date)" >> "Astle_2016_m1e6_vs_tx.log"
echo "Matched chromosomes:" >> "Astle_2016_m1e6_vs_tx.log"
...
comm -12 \
  ...
  >> "Astle_2016_m1e6_vs_tx.log" || true
...
/usr/bin/time -v python3 /gpfs/helios/home/jaanotte/gpu-coloc-workflow-portable/gpu_coloc/coloc.py \
  ...
  --results "Astle_2016_m1e6_vs_tx.tsv" \
  --p12 "1e-6" \
  --H4 "0.8" \
  >> "Astle_2016_m1e6_vs_tx.log" 2> "Astle_2016_m1e6_vs_tx.time"

echo "task_end: $(date)" >> "Astle_2016_m1e6_vs_tx.log"

if [[ ! -s "Astle_2016_m1e6_vs_tx.tsv" ]]; then
  echo -e "PP.H3\tPP.H4\tsignal1\tlead1\tsignal2\tlead2" > "Astle_2016_m1e6_vs_tx.tsv"
fi

test -s "Astle_2016_m1e6_vs_tx.tsv"
test -s "Astle_2016_m1e6_vs_tx.log"
test -s "Astle_2016_m1e6_vs_tx.time"
