...
echo "comparison_id: FinnGen_vs_tx" > "FinnGen_vs_tx.log"
echo "gwas_name: FinnGen" >> "FinnGen_vs_tx.log"
echo "eqtl_name: tx" >> "FinnGen_vs_tx.log"
echo "dir1: /gpfs/helios/projects/shared_gwas/gpu-coloc_resources/FinnGen_lbf_parquets" >> "FinnGen_vs_tx.log"
echo "dir2: /gpfs/helios/projects/shared_gwas/gpu-coloc_resources/eQTL_Catalogue/tx_parquets" >> "FinnGen_vs_tx.log"
echo "p12: 1e-6" >> "FinnGen_vs_tx.log"
echo "H4: 0.8" >> "FinnGen_vs_tx.log"
echo "task_host: $(hostname)" >> "FinnGen_vs_tx.log"
echo "task_start: $(date)" >> "FinnGen_vs_tx.log"
echo "Matched chromosomes:" >> "FinnGen_vs_tx.log"
...
/usr/bin/time -v python3 /gpfs/helios/home/jaanotte/gpu-coloc-workflow-portable/gpu_coloc/coloc.py \
  ...
  --results "FinnGen_vs_tx.tsv" \
  --p12 "1e-6" \
  --H4 "0.8" \
  >> "FinnGen_vs_tx.log" 2> "FinnGen_vs_tx.time"

echo "task_end: $(date)" >> "FinnGen_vs_tx.log"

if [[ ! -s "FinnGen_vs_tx.tsv" ]]; then
  echo -e "PP.H3\tPP.H4\tsignal1\tlead1\tsignal2\tlead2" > "FinnGen_vs_tx.tsv"
fi

test -s "FinnGen_vs_tx.tsv"
test -s "FinnGen_vs_tx.log"
test -s "FinnGen_vs_tx.time"
